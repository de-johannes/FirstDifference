...
-- Programmunfolding (nicht-rekursiv)
isJust-cons :
  ∀ (u v w : ℤ³) (rs : List ℤ³) →
...
-- False-Zweig: True muss aus dem Tail kommen
tailFromFalse :
  ∀ {u v w rs} →
...
-- Kleiner Eliminator statt 'with' im rekursiven Satz
dec-elim : ∀ {A B C : Set} → (A ⊎ B) → (A → C) → (B → C) → C
dec-elim (inj₁ a) f g = f a
dec-elim (inj₂ b) f g = g b

-- Hauptsatz: strukturelle Rekursion auf der Liste (Tail ↓)
